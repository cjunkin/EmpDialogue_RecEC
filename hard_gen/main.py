...
parser.add_argument(
    "--output-dir", type=str, default="../outputs/hard_gen",
...
def main() -> None:
    ...
    def _significance_test(epoch):
        ...
        model2.load_state_dict(torch.load("./outputs/checkpoint33.pt", map_location=device))
        model.eval()
        model2.eval()
        sample_id = 0
        all_hyps = []
    ...
    if args.do_train:
        for epoch in range(config_data.max_train_epoch):
            ...
            if F1 > 0.13 and dist2 > 0.08 and bleu4 > 0.016 and F1 > best_F1:
                best_F1 = F1
                _save_generation("_hard_gen_best")
                _save_epoch("_hard_best")
            else:
                _save_generation("_hard_gen_other")
                _save_epoch("_hard_other")
